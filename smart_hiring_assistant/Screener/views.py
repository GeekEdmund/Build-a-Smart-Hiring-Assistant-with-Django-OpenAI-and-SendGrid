    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Screen the resume with enhanced evaluation
...
def screen_resume(applicant_data, rag_service, sendgrid_service):
    ...
    try:
        ...
        # Process resume using enhanced RAG service
        ...
        # Enhanced logging with detailed assessment
        ...
        try:
            if assessment_result['meets_requirements']:
                ...
                # Send basic rejection email (removed personalization to fix argument count)
...
def format_detailed_assessment(assessment):
    ...
    try:
        # Clean and format the assessment text
...
def store_assessment_results(resume, assessment_result):
    """Store the assessment results for future reference."""
    try:
        # Assuming you have a related AssessmentResult model
        assessment_data = {
            'detailed_assessment': assessment_result.get('assessment', ''),
            'meets_requirements': assessment_result.get('meets_requirements', False),
            'raw_response': assessment_result.get('raw_response', ''),
            'created_at': datetime.now()  # Using datetime instead of timezone
        }
        
        # Store assessment data (implement based on your model structure)
        # AssessmentResult.objects.create(resume=resume, **assessment_data)
        
        logger.info(f"Assessment results stored for {resume.name}")
    except Exception as e:
        logger.error(f"Failed to store assessment results: {str(e)}")
